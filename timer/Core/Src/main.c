...
/* USER CODE BEGIN PV */
const uint16_t offsize = 5;
volatile bool rdy = false;
volatile uint16_t offset = 0;
volatile uint32_t dma_circular[10];
volatile uint32_t safe_buf[10];
volatile bool circular_tracker[10];
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_IC_Start_DMA(&htim2, TIM_CHANNEL_1, dma_circular, offsize * 2);
